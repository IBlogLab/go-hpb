...
func (ps *PeerSyn) RequestHeadersByHash(origin common.Hash, amount int, skip int, reverse bool) error{
	ps.net.Log().Debug("Fetching batch of headers", "count", amount, "fromhash", origin, "skip", skip, "reverse", reverse)
	return ps.net.SendData(p2p.GetBlockHeadersMsg, &getBlockHeadersData{Origin: hashOrNumber{Hash: origin}, Amount: uint64(amount), Skip: uint64(skip), Reverse: reverse})
}
func (ps *PeerSyn) RequestHeadersByNumber(origin uint64, amount int, skip int, reverse bool) error {
	ps.net.Log().Debug("Fetching batch of headers", "count", amount, "fromnum", origin, "skip", skip, "reverse", reverse)
	return ps.net.SendData(p2p.GetBlockHeadersMsg, &getBlockHeadersData{Origin: hashOrNumber{Number: origin}, Amount: uint64(amount), Skip: uint64(skip), Reverse: reverse})
}
func (ps *PeerSyn) RequestBodies(hashes []common.Hash) error {
	ps.net.Log().Debug("Fetching batch of block bodies", "count", len(hashes))
	return ps.net.SendData(p2p.GetBlockBodiesMsg, hashes)
}
func (ps *PeerSyn) RequestNodeData(hashes []common.Hash) error {
	ps.net.Log().Debug("Fetching batch of state data", "count", len(hashes))
	return ps.net.SendData(p2p.GetNodeDataMsg, hashes)
}
func (ps *PeerSyn) RequestReceipts(hashes []common.Hash) error {
	ps.net.Log().Debug("Fetching batch of receipts", "count", len(hashes))
	return ps.net.SendData(p2p.GetReceiptsMsg, hashes)
}
